...
exports.handler = (event) => {
  ...
  if (!event.body) {
    return Promise.resolve(processResponse(true, 'Please specify email parameters: contactEmail, name and message', 400));
  }
  const emailData = JSON.parse(event.body);

  if(!emailData.contactEmail || !emailData.name || !emailData.message) {
    return Promise.resolve(processResponse(true, 'Please specify email parameters: contactEmail, name and message', 400));
  }

  const destination = {
    ToAddresses: ['brandonschabel1995@gmail.com']
  }
